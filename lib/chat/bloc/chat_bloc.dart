...
import 'package:ParkVip/chat/common/value_string.dart';
...
class ChatBloc extends Bloc<ChatEvents, ChatStates> {
  ...
  ChatBloc({required this.chatRepository}) : super(ChatInitialState()) {
    on<GenerateTokenEvent>((event, emit) async {
      ...
      try {
        String result = "";
        //result=prefs.getString("twilio_token")??;
       // if(result==null){
         ...
         // prefs.setString("twilio_token",result);
      //  }
        if (result != "") {
    ...
    on<UpdateTokenEvent>((event, emit) async {
      ...
      try {
        ...
        String accessToken = await chatRepository.getAccessTokenFromServer({"identity": identity});
