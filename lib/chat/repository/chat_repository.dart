...
class ChatRepositoryImpl implements ChatRepository {
  ...
  Future<String> createConversation(conversationName, identity) async {
    ...
    try {
      final String result =
          await twilioChatConversationPlugin.createConversation(conversationName: conversationName, identity: identity) ??"UnImplemented Error";
      response = result;
      return response;
  ...
  Future<String> joinConversation(conversationId) async {
    ...
    try {
      final String result = await twilioChatConversationPlugin.joinConversation(
              conversationId: conversationId) ?? "UnImplemented Error";
      response = result;
      return response;
    } on PlatformException catch (e) {
      response = e.message.toString();
  ...
  Future<String> getAccessTokenFromServer(credentials) async {
    String queryParams ='?identity='+credentials['identity'];
    var response = await apiHelper.getTwilioReq('get_twilio_token', credentials['user_token'], queryParams);
